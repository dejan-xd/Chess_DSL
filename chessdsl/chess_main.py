...
from chessdsl.input_thread import InputThread
...
class ChessMain:
    ...
    def move_logic(self, move_made, animate):
        ...
        if not self.game_state.game_over and self.game_state.player_turn:
            ...
            if move_from is not None and move_to is not None:
                move = Move(move_from, move_to, self.game_state.board)
    ...
    def ai_move_logic(self):
        ...
        :return:
        """
        ai = ChessAI().find_best_move(self.game_state, self.valid_moves)  # find best move
        if ai is None:
            ai = ChessAI().find_random_move(self.valid_moves)
    ...
    def restart_logic(self, settings):
        ...
        """
        global game_state
        game_state = GameState()  # make new game_state
        self.game_state = game_state
    ...
    def new_game_logic(self, settings):
        ...
        if self.game_state.game_over:
            ...
            JsonData.write_to_json(self.json_data)
